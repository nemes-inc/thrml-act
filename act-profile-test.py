...
def generate_eeg_test_signal(length: int, fs: float) -> Tuple[np.ndarray, np.ndarray]:
  ...
  clean_signal += (
    2.0 * np.sin(2 * np.pi * 10.0 * t) * np.exp(-0.5 * ((t - 1.0) / 0.5) ** 2)
  )
...
def run_profile_test() -> None:
  ...
  print("  (THRML-ACT samples from this space rather than generating all)")
  ...
  for comp_idx in range(TRANSFORM_ORDER):
    ...
    for params in candidates:
      refined_params, corr = bfgs_refine(params, residual, t_vec, FS, max_iterations=20)
  ...
  for i, (params, coeff) in enumerate(results[:5]):  # Show first 5
    print(
      f"  {i + 1}: tc={params.tc:.1f}, fc={params.fc:.2f} Hz, "
  ...
  print(
    f"Reconstruction captures: {100 - final_residual_percent:.1f}% of signal energy"
  )
