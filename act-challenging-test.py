...
def generate_challenging_signal(
  length: int, fs: float
  ...
  ground_truth = [
    ...
    {"tc": 60.0, "fc": 4.0, "logDt": -1.0, "c": -5.0, "name": "Chirplet 2 (downchirp)"},
    {
      ...
      "logDt": 0.0,
      "c": 1.0,
      "name": "Chirplet 3 (slight upchirp)",
    },
  ]
  ...
  for gt in ground_truth:
    ...
    chirplet = generate_chirplet(
      jnp.array([gt["tc"], gt["fc"], gt["logDt"], gt["c"]]), jnp.array(t), fs
    )
...
def run_challenging_test() -> None:
  ...
  for gt in ground_truth:
    print(
      f"  {gt['name']}: tc={gt['tc']}, fc={gt['fc']} Hz, "
  ...
  print("Search ranges:")
  print(f"  tc: [0, {LENGTH}]")
  print("  fc: [0.6, 15.0] Hz")
  print("  logDt: [-4.0, 1.0]")
  print("  c: [-10.0, 10.0] Hz/s")
  ...
  for comp_idx in range(ORDER):
    ...
    print(
      f"  THRML sampling: {stage1_time:.2f}s, best correlation: {max(correlations):.4f}"
    )
    ...
    print(
      f"  BFGS refinement: {stage2_time:.2f}s, refined correlation: {best_corr:.4f}"
    )
    ...
    print(
      f"  Found: tc={best_params.tc:.1f}, fc={best_params.fc:.2f}, "
  ...
  for i, (params, coeff) in enumerate(results):
    print(
      f"  Component {i + 1}: tc={params.tc:.1f}, fc={params.fc:.2f}, "
  ...
  for i, gt in enumerate(ground_truth):
    ...
    ax1.plot(t, np.array(chirplet), "--", alpha=0.6, linewidth=1, label=f"{gt['name']}")
  ...
  for i, (params, coeff) in enumerate(results):
    chirplet = generate_chirplet(
      jnp.array([params.tc, params.fc, params.logDt, params.c]), t_vec, FS
    )
  ...
  ax3.plot(
    t, reconstruction, "r--", alpha=0.8, linewidth=1.5, label="THRML-ACT Reconstruction"
  )
  ...
  for i, gt in enumerate(ground_truth):
    ...
    ax5.plot(
      t_chirp[mask], inst_freq[mask], linewidth=3, label=f"{gt['name']}", alpha=0.8
    )
    ax5.scatter(
      [tc_sec],
      [gt["fc"]],
  ...
  for i, (params, coeff) in enumerate(results):
    ...
    ax6.plot(
      ...
      inst_freq[mask],
      linewidth=3,
      label=f"Component {i + 1}: |a|={abs(coeff):.2f}",
      alpha=0.8,
    )
    ax6.scatter(
      [tc_sec],
